...
service /reservations on new http:Listener(9090) {
    ...
    resource function post .(NewReservationRequest payload) returns Reservation|NewReservationError|error? {
        ...
        Reservation reservation = {checkinDate : payload.checkoutDate,checkoutDate: payload.checkoutDate,
         id: roomReservations.length()+1 , user: payload.user, room:availableRoom };
